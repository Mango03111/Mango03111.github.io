...
| 云服务器     | 公网 IPv4        | 119.28.119.224                                               | 腾讯云 Lighthouse                      |
...
| 固定槽位 D01 | 公网 Base URL    | https://119.28.119.224/d01/v1                                | OpenAI 兼容 API                        |
...
| 固定槽位 D02 | 公网 Base URL    | https://119.28.119.224/d02/v1                                | OpenAI 兼容 API                        |
...
| HTTPS        | 证书主体         | IP Address:119.28.119.224                                    | Let’s Encrypt 短期 IP 证书             |
...
| HTTPS        | 证书文件         | lego-prod/certificates/119.28.119.224.crt                    | OpenResty 使用                         |
| HTTPS        | 私钥文件         | lego-prod/certificates/119.28.119.224.key                    | 不得外传                               |
...
D01：
外部客户端
  → HTTPS 119.28.119.224:443/d01/v1/*
  → OpenResty 鉴权与路径重写
  → 云端 127.0.0.1:1000
...
D02：
外部客户端
  → HTTPS 119.28.119.224:443/d02/v1/*
  → OpenResty 鉴权与路径重写
  → 云端 127.0.0.1:1001
  → frps ← TLS FRP 隧道 → frpc
  → 本地 127.0.0.1:8001
...
1. 腾讯云服务器具有公网 IPv4：119.28.119.224。
...
foreach ($port in 1000, 1001, 443) {
    ...
    try {
        $ok = $client.ConnectAsync("119.28.119.224", $port).Wait(3000) -and $client.Connected
    }
    catch {
        $ok = $false
    }
...
curl --max-time 10 \
  http://119.28.119.224/.well-known/acme-challenge/vllm-check
~~~
...
~~~bash
sudo openssl x509 \
  -in /opt/1panel/apps/openresty/openresty/www/sites/AI-api/lego-prod/certificates/119.28.119.224.crt \
...
server {
    ...
    location / {
        return 308 https://119.28.119.224$request_uri;
    }
}
...
server {
    ...
    ssl_certificate /www/sites/AI-api/lego-prod/certificates/119.28.119.224.crt;
    ssl_certificate_key /www/sites/AI-api/lego-prod/certificates/119.28.119.224.key;
...
~~~text
conflicting server name "119.28.119.224" on 0.0.0.0:80, ignored
~~~

旧 Wiki 配置包含：

~~~nginx
server_name wiki2.antl.net 119.28.119.224;
~~~
...
ExecStart=/usr/local/bin/lego run --server letsencrypt --email ${LEGO_EMAIL} --accept-tos --domains 119.28.119.224 --key-type EC256 --http --http.webroot /opt/1panel/apps/openresty/openresty/root --path /opt/1panel/apps/openresty/openresty/www/sites/AI-api/lego-prod --profile shortlived --ipv4only
...
curl --max-time 15 -sS -o /dev/null \
  ...
  https://119.28.119.224/d01/v1/models

curl --max-time 15 -sS -o /dev/null \
  -w 'HTTP_STATUS=%{http_code}\n' \
  https://119.28.119.224/d02/v1/models
~~~
...
try {
    ...
    $Response = Invoke-RestMethod -Uri "https://119.28.119.224/d01/v1/models" -Headers $Headers -Method Get
...
~~~text
Base URL: https://119.28.119.224/d01/v1
...
~~~text
Base URL: https://119.28.119.224/d02/v1
